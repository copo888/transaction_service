...
func (l *PayCallBackTranactionLogic) PayCallBackTranactionForSuccess(ctx context.Context, in *transactionclient.PayCallBackRequest) (resp *transactionclient.PayCallBackResponse, err error) {
	...
	limit := utils.FloatMulCWithTrancated(order.OrderAmount, 0.05, order.CurrencyCode)
	diff := math.Abs(utils.FloatSubCWithTrancated(order.OrderAmount, in.OrderAmount, order.CurrencyCode))
...
func (l *PayCallBackTranactionLogic) updateOrderAndBalance(db *gorm.DB, req *transactionclient.PayCallBackRequest, order *types.OrderX) (err error) {
	...
	if req.OrderStatus == "20" {
		...
		order.TransferHandlingFee = utils.FloatAddCWithTrancated(utils.FloatMulC(utils.FloatDivC(order.ActualAmount, 100, order.CurrencyCode), order.Fee, order.CurrencyCode), order.HandlingFee, order.CurrencyCode)
