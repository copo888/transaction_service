...
func (l *MakeUpReceiptOrderTransactionLogic) MakeUpReceiptOrderTransaction(req *transactionclient.MakeUpReceiptOrderRequest) (*transactionclient.MakeUpReceiptOrderResponse, error) {
	...
	transferHandlingFee := utils.FloatAddCWithTrancated(utils.FloatMulC(utils.FloatDivC(req.Amount, 100, order.CurrencyCode), order.Fee, order.CurrencyCode), order.HandlingFee, order.CurrencyCode)
	// 計算實際交易金額 = 訂單金額 - 手續費
