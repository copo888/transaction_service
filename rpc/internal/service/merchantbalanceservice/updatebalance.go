...
func UpdateDFBalance_Debit(ctx context.Context, db *gorm.DB, updateBalance *types.UpdateBalance) (merchantBalanceRecord *types.MerchantBalanceRecord, err error) {
	...
	if utils.FloatAddCWithTrancated(merchantBalance.Balance, -updateBalance.TransferAmount, updateBalance.CurrencyCode) < 0 { //判斷餘額是否不足
	...
	afterBalance = utils.FloatAddCWithTrancated(beforeBalance, -updateBalance.TransferAmount, updateBalance.CurrencyCode) //代付出款固定TransferAmount代負號
...
func UpdateDF_Pt_Balance_Debit(ctx context.Context, db *gorm.DB, updateBalance *types.UpdateBalance) (merchantPtBalanceRecord types.MerchantPtBalanceRecord, err error) {
	...
	if utils.FloatAddCWithTrancated(merchantPtBalance.Balance, -updateBalance.TransferAmount, updateBalance.CurrencyCode) < 0 { //判斷餘額是否不足
